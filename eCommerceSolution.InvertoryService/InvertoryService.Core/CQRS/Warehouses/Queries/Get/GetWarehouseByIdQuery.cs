...
public class GetWarehouseByIdQueryHandler(IWarehouseRepository warehouseRepository) : IQueryHandler<GetWarehouseByIdQuery, WarehouseResponse>
{
    public async Task<Result<WarehouseResponse>> HandleAsync(GetWarehouseByIdQuery query, CancellationToken ct = default)
    {
        if (await warehouseRepository.GetByIdAsync(query.Id, ct) is not { } warehouse)
